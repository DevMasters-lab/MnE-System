...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-8 w-full px-2">
        ...
        <div class="bg-white px-4 py-2 rounded-lg border border-gray-100 shadow-sm hidden md:block">
            <p class="text-[9px] text-gray-400 uppercase font-black tracking-widest mb-1">Total Roles</p>
            <p class="text-xl font-bold text-[#1e2336] leading-none">{{ $roles->total ?? $roles->count() }}</p>
        </div>
    </div>
        ...
        <div class="px-8 py-6 border-b border-gray-50 bg-[#f8fafc] flex items-center gap-4">
            ...
            <div>
                <h3 class="text-[13px] font-bold text-[#1e2336] uppercase tracking-[0.15em]">
                    {{ isset($role) ? 'Edit Security Role' : 'Add New Role' }}
                </h3>
            </div>
        </div>
            ...
            <form action="{{ isset($role) ? route('roles.update', $role->id) : route('roles.store') }}" method="POST">
                ...
                <div class="space-y-8">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-[#f8fafc] p-4 rounded-xl border border-gray-50">
                            ...
                            @foreach($permissions as $permission)
                                <label class="group flex items-center justify-between p-4 bg-white border border-gray-100 rounded-xl cursor-pointer hover:border-blue-400 transition-all shadow-sm shadow-gray-50">
                                    ...
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" 
                                           ...
                                           class="w-5 h-5 rounded border-gray-200 text-[#4f70ce] focus:ring-[#4f70ce] focus:ring-offset-0 cursor-pointer">
                                </label>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="flex justify-end gap-6 mt-10 pt-8 border-t border-gray-50">
                    @if(isset($role))
                        ...
                        class="text-[11px] font-bold text-gray-400 hover:text-gray-600 transition uppercase tracking-[0.2em] flex items-center">
                            Cancel
                        </a>
                    @else
                        <button type="reset"
                ...
                <tr class="hover:bg-gray-50/50 transition">
                    ...
                    <td class="px-8 py-6">
                        <div class="flex justify-center gap-5">
                            <a href="{{ route('roles.index', ['edit' => $item->id]) }}" class="text-blue-500 hover:scale-110 transition-transform">
